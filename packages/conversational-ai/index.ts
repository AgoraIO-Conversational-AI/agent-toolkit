...
export class ConversationalAIAPI extends EventHelper<ConversationalAIAPIEventMap> {
  ...
  static getInstance(): ConversationalAIAPI {
    if (!ConversationalAIAPI.instance) {
      throw new Error(
        "ConversationalAIAPI not initialized. Call init() first.",
      );
    }
    return ConversationalAIAPI.instance;
  }
      ...
      try {
        ...
        if (parts.length === 4) {
          ...
          try {
            ...
            console.error(
              "[API] Failed to parse non-chunked message:",
              parseError,
            );
  ...
  private async initRTM(
    ...
    try {
      ...
      console.log(
        "💬 [API] RTM initialized and subscribed to channel:",
        config.channel,
      );
    } catch (error) {
...
export type {
  ...
  UseLocalVideoReturn,
  VideoDevice,
} from "../react/use-local-video";

export { useRemoteVideo } from "../react/use-remote-video";
export type {
  UseRemoteVideoConfig,
  UseRemoteVideoReturn,
  RemoteVideoUser,
} from "../react/use-remote-video";
